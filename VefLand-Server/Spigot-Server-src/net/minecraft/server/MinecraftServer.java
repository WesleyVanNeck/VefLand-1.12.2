    ...
    protected void l() {
        ...
        for (int m = 0; m < worlds.size(); m++) {
            ...
            runs.add(new Runnable() {
                @Override
                public void run() {
                    WorldServer worldserver = MinecraftServer.this.worlds.get(thism);
                    MinecraftServer.LOGGER.info("Preparing start region for level " + thism + " with VefLand-MultiThread(Seed: " + worldserver.getSeed() + ")");
                    ...
                    for (int k = -192; k <= 192 && MinecraftServer.this.isRunning(); k += 16) {
                        for (int l = -192; l <= 192 && MinecraftServer.this.isRunning(); l += 16) {
                            long i1 = aw();

                            if (i1 - j > 1000L) {
                                MinecraftServer.this.a_("Preparing spawn area", WorldLoadi * 100 / 625);
                                j = i1;
                            }
